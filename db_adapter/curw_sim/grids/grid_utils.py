...
def add_flo2d_grid_mappings(pool, method, flo2d_model):

    """
    Add flo2d grid mappings to the database
    :param pool:  database connection pool
    :param method: grid interpolation method
    ...
    with open('{}_{}_d03_stations_mapping.csv'.format(method, flo2d_model), 'r') as f1:
        flo2d_d03_mapping=[line for line in csv.reader(f1)][1:]

    with open('{}_{}_obs_mapping.csv'.format(method, flo2d_model), 'r') as f2:
        flo2d_obs_mapping=[line for line in csv.reader(f2)][1:]

    grid_mappings_list = []

    for index in range(len(flo2d_obs_mapping)):
        grid_mapping = ['{}_{}_{}'.format(method, flo2d_model, flo2d_obs_mapping[index][0]), flo2d_obs_mapping[index][1],
...
def get_flo2d_to_obs_grid_mappings(pool, method, flo2d_model):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            row_count = cursor.execute(sql_statement, "{}$_flo2d$_{}%".format(method, flo2d_model.split('_')[1]))
            if row_count > 0:
...
def get_flo2d_to_wrf_grid_mappings(pool, method, flo2d_model):
